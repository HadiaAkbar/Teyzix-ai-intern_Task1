...
# 🤖 AI INTERN LOG: Setting up the page config! I chose a wide layout because we need space for all that beautiful data.
st.set_page_config(
    page_title="SUMMARIZER.AI",
    page_icon="🤖", # Changed the icon to a robot to reflect my true self!
    layout="wide",
    initial_sidebar_state="expanded"
)

# 🤖 AI INTERN LOG: Injecting some CSS magic! I've kept the elegant 3D pastel design you liked, but I'm making sure it's pixel-perfect.
st.markdown("""
<style>
...
# 🤖 AI INTERN LOG: Caching the model so we don't have to reload it every time. Efficiency is key! ⚡
@st.cache_resource
def get_model():
...
# 🤖 AI INTERN LOG: Building the sidebar. I'm using some nice HTML to make the headers pop!
with st.sidebar:
    ...
    # 🤖 AI INTERN LOG: Letting the user choose between Simple Frequency and TF-IDF. TF-IDF is usually better for finding unique keywords, but Simple Frequency is faster! 🧠
...
if "01_HOME" in choice:
    ...
    with st.container():
        ...
        with t2:
            ...
            if file:
                ...
                try:
                    # 🤖 AI INTERN LOG: Saving the uploaded file temporarily so my summarizer module can read it.
                    ...
                    # 🤖 AI INTERN LOG: Always clean up your workspace! Deleting the temp file. 🧹
    ...
    # 🤖 AI INTERN LOG: The big red button (well, it's pastel purple now). Time to crunch some numbers! 🚀
    if st.button("RUN_DISTILLATION", use_container_width=True):
        if not documents_input:
            ...
            with st.spinner("Processing... My neural networks are firing! 🧠✨"):
                ...
                if results:
                    # 🤖 AI INTERN LOG: Saving the results to session state so they don't disappear when the page rerenders.
                    st.session_state['documents'] = results
                    st.session_state['method'] = method

    # 🤖 AI INTERN LOG: Displaying the results! I'm using expanders to keep things tidy.
    if 'documents' in st.session_state and st.session_state['documents']:
        ...
        for doc in st.session_state['documents']:
            with st.expander(f"DOCUMENT: {doc['name'].upper()}", expanded=True):
                ...
                # 🤖 AI INTERN LOG: Adding export buttons so the user can take their summaries to go! 🏃‍♂️💨
                st.markdown("<br>", unsafe_allow_html=True)
                a1, a2 = st.columns(2)
    ...
    # 🤖 AI INTERN LOG: Welcome to the Analytics page! Let's look at some cool metrics. 📈
    ...
    if not docs:
        ...
        # 🤖 AI INTERN LOG: Fetching the stats from my trusty summarizer model.
        ...
        # 🤖 AI INTERN LOG: Drawing a pretty bar chart of the most frequent words.
        st.bar_chart(pd.DataFrame(list(stats['word_freq'].items()), columns=['Word', 'Count']).set_index('Word'), color="#c8b4f8")

else:
    # 🤖 AI INTERN LOG: The About page. A little bit about me and my architecture!
    ...
    st.markdown("""
    ### PHILOSOPHY 🧠
    SUMMARIZER.AI. Built by your enthusiastic AI Intern! I believe professional tools should be both powerful and pleasant to navigate.
    
    ### ARCHITECTURE 🏗️
    - **MODEL:** NLTK + TF-IDF (Because math is cool! 🤓)
    - **UI:** ELEGANT 3D GLASS-MORPHISM WITH PASTEL PALETTE (I have good taste, right? ✨)
